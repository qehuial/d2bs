23.07.30
1. 아이템 식별 책을 안들고 있을 경우 인벤토리가 꽉 찼을 때 미확인 스크롤을 사지 못하는 현상 수정

   -> 미확인 아이템 중에서 가장 왼쪽 위에 있는 하나를 판매하여 자리를 확보 후 진행
   
   -> 인벤토리 설정(아래 스샷)에 따른 영향 받음
   ![1](https://github.com/qehuial/d2bs/assets/100043245/ca3653ba-d0cd-4698-a9d9-7e786637603a)
...
Config.BeltColumn에 "tsc"만 입력하면 되도록 변경 필요없어진 Config.TownPortal 삭제

1. 변경 전 : 벨트에 포탈이 1개 남을 경우 남은 포탈 사용해서 마을로 이동하여 포탈 구매 후 사냥 진행
...
4. 스크롤이 1개가 남았을 때 마을로 이동하여 포탈스크롤 구매 - [23년 6월 10일자로 수정됨]

(이 부분은 2개 있을때 사용해서 1개만 남아버리면 바로 소모하여 마을로 이동하여 구매합니다.) > 마지막에 열었던 포탈로 이동하게 수정됨
